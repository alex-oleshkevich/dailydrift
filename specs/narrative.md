...
- **Not the Curator/agents.** The actor that generates Narratives is owned by [agents](agents.md) (the Memory-Curator role); this spec owns *what* it must produce, not how the agent is orchestrated.
...
> **REQ-NAR-04.** A Narrative is **generated by the Curator** — the downstream Memory-Curator role ([agents](agents.md)) — over the scope's **Evidence, Situations, Insights, and Storylines** (and, for a Space, its Storylines' Narratives). The Curator continuously asks *"what story best explains all of this?"* It is **never** generated by a [Signal](signals.md) or the [Inbox](inbox.md) directly: the Inbox commits Evidence and *triggers* a Curator job (REQ-INBOX-10/12), which is where the Narrative is (re)written. Generating internal synthesis is an **Always** action ([constitution](constitution.md) §5).
...
- [memory](memory.md) — capture/retention/recall; the Narrative is the synthesis surface alongside it. [agents](agents.md) — the Memory-Curator that generates it. [home-and-briefings](home-and-briefings.md) / [conversation](conversation.md) / [proactivity](proactivity.md) — the surfaces.
