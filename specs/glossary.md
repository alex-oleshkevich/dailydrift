...
> **Status:** Approved
>
> **Version:** 1.8   ·   **Last updated:** 2026-06-09
>
...
A system that "remembers, connects, and continues" needs a stable, shared language. If "Storyline" means one thing in `narrative` and another in `insights`, the product fractures. One glossary, owned here, keeps every spec — and the eventual code — speaking the same words.
...
- **Status** — the lifecycle state of a Storyline or Situation, drawn from a shared vocabulary but **per-type**: a Storyline is *active · blocked · resolved · dormant*; a Situation is *active · blocked · resolved · snoozed · dismissed* ([situations](situations.md) REQ-SIT-07, [data-model](data-model.md) §5.6). Momentum is orthogonal to Status (OQ-CON-3).
...
- **Digest** — a **periodic roll-up briefing** (daily · weekly · space · blocked-work · insight). *→ rendered by the client; policy in [proactivity](proactivity.md).*
...
| Narrative | Storyline, Situation, Momentum, Attention score, Status | [glossary](glossary.md) (here) + surfaced by the client (out of scope) |
...
| Surfaces | Conversation, Message, Digest | [conversation](conversation.md), client (out of scope) |
...
- **2026-06-09 — v1.8** — Corrected the **Status** entry: it had listed a single `active · blocked · resolved · dormant` set that matched neither primitive. Status is **per-type** — Storyline (`…· dormant`) vs Situation (`…· snoozed · dismissed`, [situations](situations.md) REQ-SIT-07) — drawn from a shared vocabulary, with Momentum orthogonal (OQ-CON-3, [data-model](data-model.md) §5.6).
